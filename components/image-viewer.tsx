...
const ImageViewer = ({ url, index }: Props) => {
    ...
    return (
        <div key={index} className="relative group aspect-w-16 aspect-h-9 bg-transparent hover:scale-110 transition-all ease-in-out duration-300 "> {/* Adjust aspect ratio as needed */}
            ...
            <Image
                ...
                height={300}
                alt={url || `Image ${index}`}
                className={' w-full h-full object-scale-down' + (loading ? ' blur-sm animate-pulse bg-slate-700 ' : ' ')}
